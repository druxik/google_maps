							...
							<template v-if="s.type=='route'">
								<div class="rp-layer-route rp-layer-draggable" :key="s.id" :ref="'shape'+s.id">
									...
									<div class="clearfix" v-if="s.editing">
										...
										<span class="icon-times close-picker" @click="closePicker(s)" v-show="test(s.showColor)"></span>
										<chrome-picker v-bind:value="colors" @input="updateValue(colors, s)" v-show="s.showColor" v-model="colors"></chrome-picker>
										
										<div class="input-prepend input-append" style="display: flex;display: -webkit-flex;    margin-bottom: 4px;" v-show="test(!s.showColor)">
											<div style="width: 72px;height: 30px; margin-bottom: 4px;margin-left: 1px;;display: inline-block; border-top-left-radius: 5px; border-bottom-left-radius: 5px;" v-bind:style="{background: s.options.color}"></div>
											<input class="form-control" type="text" name="" style="display: inline-block;width: 173px;height: 30px; border-top-left-radius: 0; border-bottom-left-radius: 0;" v-model="s.options.color">
											<button class="btn" type="button" style="height: 30px;width: 30px;padding: 0;" @click="showPicker(s)">
												<span class="icon-pencil close-picker" style="font-size: 20px;"></span>
											</button>
										</div>
									</div>
									<div class="clearfix" v-if="s.editing">
